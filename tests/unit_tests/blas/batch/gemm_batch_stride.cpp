...
int test(device* dev, oneapi::math::layout layout, int64_t batch_size) {
    ...
    // its error against an exact integer reference is only the final scaling. The error-model test
    // checks that path with a tighter bound. Against CBLAS the looser fp32 bound still applies.
